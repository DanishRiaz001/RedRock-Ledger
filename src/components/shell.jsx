...
function SignedFileViewer({storagePath,type,name,style}){
  ...
  return(
    <div style={{...style,padding:0,display:"flex",flexDirection:"column"}}>
      <Toolbar/>
      <div onWheel={onWheel} style={{position:"relative",overflow:"hidden",background:"#fafafa",flex:1}}>
  ...
  return(
    <div style={{display:"flex",alignItems:"stretch",gap:0,minWidth:0}}>
      ...
      {collapsible&&collapsed?(
        <div onClick={()=>setCollapsed(false)} title={expandLabel} style={tabStyle}>
          <i className="ti ti-chevron-left" style={{fontSize:12,transform:"rotate(90deg)"}}/>{expandLabel}
        </div>
      ):(<>
        <div onMouseDown={startDrag} title="Drag to resize" style={{width:8,alignSelf:"stretch",cursor:"col-resize",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
          <div style={{width:3,height:40,borderRadius:2,background:"#E4EDED"}}/>
        </div>
        <div style={{width:rightWidth,flexShrink:0,minWidth:minRightWidth,position:"relative"}}>
          {right}
          {collapsible&&(
