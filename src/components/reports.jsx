  ...
  return(
    <div>
      ...
      {/* A wide plain table stretched every column to fill the row, which
          ...
          SAME grid-template-columns, so every piece of information —
          period, due date, amount, status, actions — lines up into a real
...
function BankReconciliationScreen({accounts,contacts,transactions,bankStatementLines,uploadBankStatement,parseBankStatementFile,parseBankStatementPDF,commitBankStatementRows,undoBankImport,postBankStatementLine,postBankStatementLinesBulk,deleteBankStatementLine,matchBankStatementLine,unmatchBankStatementLine,toggleReconciled,onEditTxn,onDeleteTxn,onReverseTxn,fetchTxnAttachments,uploadInboxFile,attachFilesToTxnEntry,onRemoveAttachment,onCreateAccount,onCreateContact,inboxFiles=[],fetchEntryComments,addEntryComment,auditLog,profiles,currentUserId,moneySources,projects=[],tagTransaction,attachments={},onAttach,onRemoveAttach,addTransaction,onSaveAccounts,onNavigate,attachedTxnIds=[],attachedFileIds=[]}){
  ...
  // The three summary cards (Entered in ledger / Selected / Ready to
  // book) render OUTSIDE the ResizableSplit's own left pane — same as the
  // toolbar above — so they never actually shrank when the attachment
  // panel (position:fixed, pinned to the true window edge) opened. The
  // third card, "Ready to book", is exactly where the Post button lives;
  // with no margin compensation it ended up rendered right under the
  // panel — present in the DOM, invisible and unclickable. Same fix as
  // toolbarActionsRef below, via extraMarginRefs.
  const summaryCardsRef=React.useRef(null);
  ...
  return(
    <div>
      ...
      {(()=>{
        ...
        return(
          <div ref={summaryCardsRef} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:16}}>
      ...
      {bulkPostOpen&&(()=>{
        ...
        // Which side the selected lines mostly are — outgoing (money left
        // the account) or incoming — decides which payment-type group
        // leads. Both groups are still shown, just ordered by relevance.
        const outCount=selectedLines.filter(l=>l.amount<0).length;
      ...
      {detailTxn&&<DetailModal txn={detailTxn} initialShowEdit addTransaction={addTransaction} accounts={accounts} contacts={contacts} transactions={transactions}
        ...
        auditLog={auditLog} profiles={profiles} currentUserId={currentUserId} moneySources={moneySources} projects={projects} tagTransaction={tagTransaction}
