...
function OpeningBalanceScreen({accounts,contacts,setContacts,transactions,addTransaction,onSave,onBack,uploadInboxFile}){
  ...
  const doUploadAttachment=async(file)=>{
    if(!uploadInboxFile)return;
    setUploadingAttachment(true);
    const uploaded=await uploadInboxFile(file,"Opening Balance");
...
function ProjectTrackingScreen({companyProfile,saveCompanyProfile,projects,saveProjects,transactions=[],onBack}){
  ...
  // Usage count per project — a real signal Tripletex's own project list
  // doesn't surface: which projects are actually being used day to day vs.
  // sitting dormant, so it's obvious which ones are safe to deactivate.
  const usageCount=id=>transactions.filter(t=>t.projectId===id).length;
    ...
    <div style={{maxWidth:700}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>
        {[["all","All"],["active","Active"],["inactive","Inactive"]].map(([id,label])=>(
          <button key={id} onClick={()=>setStatusFilter(id)} style={{background:statusFilter===id?T.accent:"none",color:statusFilter===id?"#fff":T.sub,border:`1px solid ${statusFilter===id?T.accent:T.border}`,borderRadius:8,padding:"6px 13px",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>{label}</button>
        ))}
        <div style={{flex:1}}/>
        <input placeholder="Search" value={search} onChange={e=>setSearch(e.target.value)} style={{...inp,width:160,padding:"7px 10px",fontSize:12}}/>
      </div>

      <div style={{border:`1px solid ${T.border}`,borderRadius:12,overflow:"hidden"}}>
        {!visibleProjects.length&&<div style={{padding:20,textAlign:"center",fontSize:12,color:T.muted}}>{projects.length?"No projects match.":"No projects yet — add one above."}</div>}
        {visibleProjects.map((p,i)=>{
          const uses=usageCount(p.id);
          return(
          <div key={p.id} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",borderBottom:i<visibleProjects.length-1?`1px solid ${T.border}`:"none",opacity:p.inactive?0.5:1}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,fontWeight:600,color:T.text}}>{p.name}{p.inactive&&" (inactive)"}</div>
              <div style={{fontSize:10,color:T.muted,marginTop:1}}>{uses} {uses===1?"entry":"entries"} tagged</div>
            </div>
